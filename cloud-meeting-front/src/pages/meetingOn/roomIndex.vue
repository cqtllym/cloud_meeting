<template>
  <div class="content">
    <div class="chatroom">
      <div class="title">
        会议室会议号:&nbsp; {{ roomMeetingId }} ({{ meetingCount }})
      </div>
      <div class="fix"></div>
...
import { confirmRoomReservation, getRoomReservation } from "@/services/reservation";
// 引入安装包和样式
...
export default {
  ...
  created() {
    this.roomMeetingId = this.$route.query.roomMeetingId
    // this.scheduledEndTime= this.$route.query.scheduledEndTime
    confirmRoomReservation(this.roomMeetingId).then(res => {
      if (res.data.code === 708) {
        ...
        if (res.data.data.confirmResult) {
          let data = {
            id: this.roomMeetingId
          }
          getRoomReservation(data).then(res => {
            this.hostId = res.data.data.hostId
            this.scheduledEndTime = res.data.data.scheduledEndTime
            this.msg = JSON.parse(sessionStorage.getItem("msg" + this.roomMeetingId)) ? JSON.parse(sessionStorage.getItem("msg" + this.meetingNumber)) : []
          })
        } else {
          this.$message.error('入会无效！')
        }
      }
      // }
    })
  ...
  mounted() {
    console.log(this.roomMeetingId)
    window.addEventListener('beforeunload', e => {
      console.log(e);
      let key = "msg" + this.roomMeetingId;
    ...
    //普通会议 room改成meeting  
    this.ws = new WebSocket('ws://139.9.119.138:30000/room/' + this.roomMeetingId + '/' + userId + '/' + this.username);
    console.log('ws://139.9.119.138:30000/room/' + this.roomMeetingId + '/' + userId + '/' + this.username)
  ...
  methods: {
    ...
    checkTime() {
      let now = new Date()
      if (this.roomMeetingId != "" && this.count == 0 && now.getTime() >= this.scheduledEndTime) {
        this.count++
        this.$notification['error']({
          message: '会议已结束！',
          description:
