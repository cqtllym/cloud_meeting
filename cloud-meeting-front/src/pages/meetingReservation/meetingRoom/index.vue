<template>
  ...
  >
    ...
    <a-modal :visible="result_visible" @cancel="handleCancel" :footer="null">
      <a-result
        status="success"
        :title="'预约成功（会议室会议ID为 ' + roomMeetingId + '） ！'"
...
import { avaliableRoom, createRoomReservation, getEmployeeList } from '@/services/reservation.js'
...
export default {
  ...
  data() {
    this.lastFetchId = 0;
    this.fetchUser = debounce(this.fetchUser, 800);//防抖
    return {
      current: 0,
      form: {
        topic: '',
  ...
  methods: {
    ...
    handleChange(participants) {
      this.participantsIdList = []
      for (let i = 0; i < participants.length; i++) {
        this.participantsIdList.push(participants[i].key)
      }
      Object.assign(this, {
        participants,
        employeeList: [],
    ...
    handleRoomReservation() {
      ...
      createRoomReservation(data).then(res => {
